    ...
    public partial class _Default : Page
    {
        ...
        async protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                   DatabaseInsert(conn, "INSERT INTO [dbo].[Logs_Access] ([AccessDateTime],[DeviceName],[DeviceIP],Browser) VALUES('" + DateTime.Now.ToString() + "',N'" + System.Net.Dns.GetHostEntry(HttpContext.Current.Request.UserHostName.ToString()).HostName + "','"+ HttpContext.Current.Request.UserHostName.ToString() + "','"+ HttpContext.Current.Request.Browser.Browser+ "')") ;
                }
                catch { DatabaseInsert(conn, "INSERT INTO [dbo].[Logs_Access] ([AccessDateTime],[DeviceName],[DeviceIP],Browser) VALUES('" + DateTime.Now.ToString() + "',N'" + HttpContext.Current.Request.UserHostName.ToString() + "','" + HttpContext.Current.Request.UserHostName.ToString() + "','" + HttpContext.Current.Request.Browser.Browser + "')"); }
                ...
                DataTable dtReviews = DatabaseQuery(conn, "Select * from Reviews Where [Date] > '"+DateTime.Now.AddDays(-7).ToString("yyyy-MM-dd")+"'");
                string jsonResponse = "";
        ...
        protected void Calendar1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                DataTable dtReservation = DatabaseQuery(conn, "Select * From Reservation right join Reservation_Accommodation on Reservation.ID = Reservation_Accommodation.Reservation_ID inner join Accommodation on Accommodation.ID = Reservation_Accommodation.Accommodation_ID  Where '" + Calendar1.SelectedDate.ToString("yyyy-MM-dd") + "' >= CheckinDate AND '" + Calendar1.SelectedDate.ToString("yyyy-MM-dd") + "' < CheckoutDate");
                DataTable dtAccommodation = DatabaseQuery(conn, "Select * From Accommodation Where Status = 1 order by OrderID asc");
                try
                {
        ...
        public int DatabaseInsert(string connectionString, string query)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    // Check if this is an INSERT with SCOPE_IDENTITY query
        ...
        public string createDocNumber(string conn, string tablename, string doctype,string Year,string Month,string Day)
        {
            ...
            DataTable dt = DatabaseQuery(conn, "select Top 1 ID from " + tablename + " Where ID like '" + doctype + Year + Month + Day + "%' order by ID desc");
            int Number = 0;
            string NumberStr = "";
            if (dt.Rows.Count > 0)
            {
        ...
        protected void Calendar1_DayRender(object sender, DayRenderEventArgs e)
        {
            if (GridView1.SelectedIndex >= 0)
            {
                if(DateTime.Now.AddDays(-1) <= e.Day.Date)
                {
                    ...
                    DataTable dtAccommodation = DatabaseQuery(conn, "Select * From Accommodation Where Status = 1 AND AccomName = N'" + GridView1.SelectedRow.Cells[1].Text +"'");
                    for (int i = 0; i < dtReservation.Rows.Count; i++)
                    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    DataTable dtReservation = DatabaseQuery(conn, "Select * From Reservation right join Reservation_Accommodation on Reservation.ID = Reservation_Accommodation.Reservation_ID Where '" + e.Day.Date.ToString("yyyy-MM-dd") + "' >= CheckinDate AND '" + e.Day.Date.ToString("yyyy-MM-dd") + "' < CheckoutDate");
                    DataTable dtAccommodation = DatabaseQuery(conn, "Select * From Accommodation Where Status = 1 order by ID asc");
